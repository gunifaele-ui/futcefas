  ...
  return (
    <BottomSheet onClose={onClose}>
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <h3 className="text-[15px] font-semibold text-fc-ink">Mudar nota de {player.nome}</h3>
      </div>
      ...
      <div className="space-y-3">
        {admins.map((adm, idx) => {
          ...
          return (
            <div
              key={adm.key}
              className={`p-3 rounded-xl border ${isEditable ? 'border-fc-line bg-fc-surface' : 'border-fc-line bg-fc-cream/60 opacity-60'}`}
            >
              <div className="flex justify-between items-center mb-2.5">
                ...
                <div className="flex items-center gap-1.5">
                  {hasValue ? (
                    <span className="text-[13px] font-semibold text-fc-ink bg-fc-cream px-2.5 py-0.5 rounded-full min-w-[2.75rem] text-center">
                      {value.toFixed(1)}
                    </span>
                  ) : (
        ...
        <div className="flex justify-between items-center border-t border-fc-line pt-3">
          ...
          <span className="text-[14px] font-semibold text-fc-ink bg-fc-limesoft px-3 py-1 rounded-full">{media}</span>
        </div>
      </div>
