...
export default function RatingModal({ player, tempNotes, setTempNotes, onSave, onClose }) {
  ...
  return (
    <BottomSheet onClose={onClose}>
      ...
      <p className="text-[11px] text-slate-400 mb-4 font-bold">Arraste pra ajustar as notas de Gustavo, Enzo e Miguel (1.0 a 10.0):</p>

      <div className="space-y-4">
        {ADMS.map((adm) => (
          <div key={adm.key} className={`p-3 rounded-2xl border ${adm.bg} ${adm.border}`}>
            <div className="flex justify-between items-center mb-2">
              <span className={`text-xs font-black ${adm.text}`}>{adm.label}</span>
