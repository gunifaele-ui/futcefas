...
export default function ActivityLogModal({ activityLog, admins, onClose }) {
  ...
  return (
    <BottomSheet onClose={onClose}>
      <h3 className="text-[15px] font-semibold text-fc-ink mb-3.5 flex items-center gap-2">
        <Icon name="bell" size={16} className="text-fc-ink/70" /> Notificações
      </h3>
        ...
        <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-2">
                <p className="text-[12px] font-medium text-fc-ink leading-snug">
                  {entry.compiledMessage ? (
                    ...
                    <>
                      <span className="text-fc-ink/60">{getAdminLabel(admins, entry.adminKey) || 'Alguém'}</span> {entry.message}
                    </>
                  )}
                </p>
              </div>
          ...
          {visibleCount < compiledLog.length && (
            <button
              ...
              className="w-full text-[12px] font-medium text-fc-ink/70 bg-fc-cream hover:bg-fc-line py-2.5 rounded-xl transition"
            >
              Ver mais
            </button>
          )}
