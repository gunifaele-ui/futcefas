...
export default function BottomNav({ isAdmin, activeTab, onChangeTab }) {
  ...
  return (
    <>
      ...
      <nav
        className="md:hidden fixed bottom-3 left-3 right-3 max-w-md mx-auto bg-fc-surface/95 backdrop-blur border border-fc-line rounded-3xl shadow-nav flex items-center px-1.5 py-1.5 z-40"
        style={{ marginBottom: 'env(safe-area-inset-bottom)' }}
      >
      {tabs.map((tab) => {
        const active = activeTab === tab.key;
        return (
          <div key={tab.key} className="flex-1 flex justify-center">
            <button
